...
class AbstractTrainer(object):
    ...
    def get_log_paths(self):
        ...
        if not os.path.exists(self.opt.save_path):
            print("creating folder  ", self.opt.save_path)
            os.mkdir(self.opt.save_path)

        self.opt.logname = os.path.join(self.opt.save_path, "log.txt")
        self.opt.checkpointname = os.path.join(self.opt.save_path, 'checkpointname.txt')

        # # If a network is already created in th
        if os.path.exists(os.path.join(self.opt.save_path, "network.pth")):
            self.opt.model = os.path.join(self.opt.save_path, "network.pth")
    ...
    def save_network(self):
        print("saving net...")
        torch.save(self.network.state_dict(), f"{self.opt.save_path}/network.pth")
        torch.save(self.optimizer.state_dict(), f"{self.opt.save_path}/optimizer_last.pth")
    ...
    def dump_stats(self):
        ...
        """
        log_table = {
            "lambda_cycle_2": self.opt.lambda_cycle_2,
            "lambda_cycle_3": self.opt.lambda_cycle_3,
            "lambda_reconstruct": self.opt.lambda_reconstruct,
            "lambda_chamfer": self.opt.lambda_chamfer,
            "cat": self.opt.cat,
