...
class MetaPointGenCon2(nn.Module):
    ...
    def __init__(self, bottleneck_size=2500, hidden_sizes=[256, 256, 256],
        ...
        hidden_layer['fc_0'] = Meta_FC_Layer_2(3, hidden_sizes[0], cursor_Wstart=0, cursor_Wend=9, cursor_Bstart=9,
                                               cursor_Bend=12, batchnorm=batchnorm)
        cursor = 12
...
class AE_Meta_AtlasNet(nn.Module):
    def __init__(self, num_points=6890, encoder_type="Pointnet",
                 bottleneck_size=1024, nb_primitives=1,
                 hidden_sizes=[64, 64, 64, 64, 64], resnet_layers=True,
                 skip_connections=False, batchnorm=True):
        self.hidden_sizes = hidden_sizes
        self.resnet_layers = resnet_layers
        ...
        self.point_encoder_1 = PointNetfeat(
            num_points, global_feat=True, trans=False, batchnorm=batchnorm)
        self.point_encoder_2 = PointNetfeat(
            num_points, global_feat=True, trans=False, batchnorm=batchnorm)

        if batchnorm:
            self.encoder = nn.Sequential(
                self.point_encoder_1, nn.Linear(1024, 512), nn.BatchNorm1d(512),
                nn.ReLU())
            self.encoder2 = nn.Sequential(
                self.point_encoder_2, nn.Linear(1024, 512), nn.BatchNorm1d(512),
                nn.ReLU())
        else:
            self.encoder = nn.Sequential(
                self.point_encoder_1, nn.Linear(1024, 512), nn.ReLU())
            self.encoder2 = nn.Sequential(
                self.point_encoder_2, nn.Linear(1024, 512), nn.ReLU())

        self.decoder = MetaPointGenCon2(
            bottleneck_size=self.bottleneck_size, hidden_sizes=hidden_sizes,
